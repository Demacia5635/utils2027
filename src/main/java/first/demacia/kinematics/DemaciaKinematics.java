...
public class DemaciaKinematics {
    ...
    // currentVelocities) {
    // if(Math.abs(wantedVelocities.vxMetersPerSecond) < 0.05 &&
    // Math.abs(wantedVelocities.vyMetersPerSecond) < 0.05 &&
    // Math.abs(wantedVelocities.omegaRadiansPerSecond)>0.01){
    // SwerveModuleState[] rotationStates = new SwerveModuleState[4];
    // for(int i = 0; i < 4; i++){
    // rotationStates[i] = new SwerveModuleState(wantedVelocities.omegaRadiansPerSecond
